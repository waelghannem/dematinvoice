...
@RestController
@RequestMapping("/api/dematinvoice-kafka")
public class DematinvoiceKafkaResource {

    private final Logger log = LoggerFactory.getLogger(DematinvoiceKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public DematinvoiceKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
