...
import com.demat.invoice.config.KafkaSseConsumer;
import com.demat.invoice.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class DematinvoiceKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/dematinvoice-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/dematinvoice-kafka/consume")
